<template>
  <div>
    ...
    <h1 class="text-center font-extrabold text-5xl mt-4 bg-blue-300">
      Book Management System Welcomes You !!
    </h1>
    <hr class="mt-5 border-1 border-black" />
    <!-- import MyComp from "@/components/MyComp.vue"; -->
    <div
      class="sm border-4 bg-gray-100 mr-60 ml-60 p-4 border-solid border-black rounded-md mt-6"
    >
      <form
        action=""
        name="myForm"
      ...
      >
        ...
        <div class="justify-between p-1 sm">
          <label class="font-semibold text-xl mr-16" for="bookname"
            >Book Name</label
          >
          <input
            class="ml-16 border-2 px-32"
            type="text"
        ...
        <div class="p-1">
          <label class="font-semibold text-xl mr-16" for="bookauthor"
            >Book Author</label
          >
          <input
            class="ml-16 border-2 px-32"
        ...
        <div class="p-1">
          <label class="font-semibold text-xl mr-16" for="bookprice"
            >Book Price</label
          >
          <input
            class="ml-8 border-2 px-32"
            type="number"
        ...
        <div class="p-1">
          <label class="font-semibold text-xl mr-16" for="bookimage"
            >Book Image</label
          >
          <input
            class="ml-16 border-2 px-32"
            type="text"
            v-model="formdata.bookimage"
            name="bookimage"
            id="bookimage"
        ...
        <div class="p-1">
          <label class="font-semibold text-xl mr-16" for="bookisbn"
            >Book ISbn No.</label
          >
          <input
            class="ml-16 border-2 px-32"
            ...
            name="bookisbn"
            id="bookisbn"
            @change="validationBookisbn"
            required
          />
        </div>
        <div class="p-1 mt-6">
          <button
            class="ml-16 border-2 font-bold mr-16 rounded-md bg-black text-blue-500 hover:bg-black hover: text-white p-2 text-center"
            type="submit"
            @click="addpro"
          >
            Add Book
          ...
          <button
            class="ml-16 border-2 font-bold mr-16 rounded-md bg-black text-blue-500 hover:bg-black text-white p-2 text-center"
            type="reset"
          >
            Reset
          </button>
      ...
      <table
        border="2"
        class="border-2 bg-gray-100 mr-60 ml-32 p-6 border-solid border-black rounded-md mt-8"
      >
        <tr>
          <td
            class="border-2 bg-gray-100 p-2 border-solid border-black rounded-md mt-8 font-bold"
          >
            Book ID
          </td>
          <td
        ...
        <tr v-for="(item, i) in myarr" :key="item">
          <td
            class="border-2 bg-gray-100 p-2 border-solid border-black rounded-md mt-8"
          >
            {{ (item.bookid = i + 1) }}
          </td>
          <td
          ...
          >
            <button
              class="border-2 font-bold rounded-md bg-green-500 hover:bg-black text-white p-2 text-center"
              @click="deletepro(i)"
            >
              Delete
            </button>
      ...
      <div>
        <label class="font-bold text-xl mr-16" for="searchpro"
          >Search Books</label
        >
        <input
          @keyup="userFindByAddress(userAddress)"
          v-model="this.userAddress"
          class="ml-2 mt-8 border-2 px-32"
          type="text"
          name="searchpro"
          id="searchpro"
        />
...
export default {
  ...
  methods: {
    addpro(event) {
      ...
      //validation bookname
      if (
        !isNaN(this.formdata.bookname) ||
        this.formdata.bookname == null ||
      ...
      // //vali author
      if (
        !isNaN(this.formdata.bookauthor) ||
        this.formdata.bookauthor == "" ||
        this.formdata.bookauthor == null
      ) {
      ...
      //vali Price
      if (
        isNaN(this.formdata.bookprice) ||
        this.formdata.bookprice == null ||
        this.formdata.bookprice == ""
      ) {
      ...
      //vali Image
      if (
        !isNaN(this.formdata.bookimage) ||
        this.formdata.bookimage == null ||
        this.formdata.bookimage == ""
      ) {
      ...
      if (
        isNaN(
          this.formdata.bookisbn ||
            this.formdata.bookisbn == null ||
            this.formdata.bookisbn == ""
        )
      ) {
    ...
    userFindByAddress(userAddress) {
      ...
      this.userFound = this.myarr.filter((e) => {
        ...
        if (
          ...
          this.bookimage2.startsWith(this.bookimage1) ||
          this.bookisbn2.startsWith(this.bookisbn1)
        ) {
          console.log(e);
          return e;
        }
